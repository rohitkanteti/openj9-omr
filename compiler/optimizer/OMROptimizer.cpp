...
void traverse_cfg(J9Method *method, PointerAssignmentGraph *pag, int methodIndex, TR::Compilation *comp, PAGNode *primitive_node, PAGNode *);
...
bool searchForOveridingMethodsInClass(std::string className, std::string method_name, std::string method_signature, PointerAssignmentGraph *pag, TR_J9VMBase *fej9, TR_ResolvedMethod *resolvedMethod, vector<set<PAGNode *>> actual_params, int bci, TR::Compilation *comp, operandStack *, PAGNode *, PAGNode *, std::string);
void executeBytecode(TR_J9ByteCode bytecode, uint8_t *pc, PointerAssignmentGraph *pag, operandStack *stack, TR_ResolvedMethod *resolvedMethod,
                     J9Method *currentMethod, int methodIndex, int bci, std::unordered_map<int, PAGNode *> &variableMap, bool hasReturnType, TR::Compilation *comp, J9Class *, PAGNode *primitive_node, PAGNode *);
...
// Callgraph data structures
static std::unordered_map<int, std::unordered_set<int>> callsiteBCI_to_targets;   // BCI -> target method indices
static std::unordered_map<int, std::set<PAGNode *>> callsiteBCI_to_actual_params; // BCI -> actual parameter PAG nodes
static std::unordered_map<int, PAGNode *> callsiteBCI_to_return_node;             // BCI -> return receiver node (or nullptr for void)
...
void processClinits(TR::Compilation *comp)
{
   ...
   while (gzgets(inputFile, buffer, sizeof(buffer)) != NULL)
   {
      ...
      processedClinit.insert(class_name);
                     ...
                     J9Method *currentMethod, int methodIndex, int bci, std::unordered_map<int, PAGNode *> &variableMap, bool hasReturnType, TR::Compilation *comp, J9Class *J9currentClass, PAGNode *primitive_node, PAGNode *comp_type_2,std::string currentMethodFullName)
{
   ...
   case J9BCdupx2:
   {
      ...
      for (auto frame : value2)
      {
         if (frame.isRefOfClass("COMP_TYPE_2"))
         {
            value2IsCategory2 = true;
            break;
         }
      }

      if (value2IsCategory2)
      {
         // Form 2: ..., value2, value1 -> ..., value1, value2, value1
         // where value2 is category 2 and value1 is category 1
         stack->push(value1);
         stack->push(value2);
         stack->push(value1);
      }
   ...
   case J9BCdup2x2:
   {
      ...
      if (value1IsCategory2)
      {
         ...
         if (value2IsCategory2)
         {
            ...
            // where value1 and value2 are both category 2
            stack->push(value1);
            stack->push(value2);
            stack->push(value1);
         }
      ...
      else
      {
         ...
         for (auto frame : value3)
         {
            if (frame.isRefOfClass("COMP_TYPE_2"))
            {
               value3IsCategory2 = true;
               break;
            }
         }

         if (value3IsCategory2)
         {
            // Form 3: ..., value3, value2, value1 -> ..., value2, value1, value3, value2, value1
            // where value1, value2 are category 1 and value3 is category 2
            stack->push(value2);
            stack->push(value1);
            stack->push(value3);
            stack->push(value2);
   ...
   case J9BCastorew: // NEWLY ADDED
   {
      ...
      for (PAGNode *obj_ref : stack_top)
      {
         pag->addEdge(obj_ref, variableMap[index], (obj_ref->type == OBJECT ? NEW : ASSIGN), bci);
      }
      break;
   }
   case J9BClload:
   ...
   case J9BCldcw:
   {
      ...
      case J9CPTYPE_LONG:
      {
         stack->pushRef(comp_type_2); // push dummy long constant
         break;
      }
      case J9CPTYPE_FLOAT:
      {
      ...
      {
         stack->pushRef(comp_type_2); // push dummy double constant
         break;
      }
      default:
         break;
   ...
   case J9BCgetfield:
   {
      ...
      pag->PAG_nodes.insert(temp_node_ptr);
      pag->methodIndex_to_allMethodNodes[methodIndex].push_back(temp_node_ptr);
      if (strcmp(fieldSig, "J") == 0 || strcmp(fieldSig, "D") == 0)
      {
         temp_node_ptr->comp_type = "COMP_TYPE_2";
      }
   ...
   case J9BCinvokedynamic:
   {
      ...
      U_16 callSiteIndex = pc[1];

      J9ROMNameAndSignature *nameAndSig =
          SRP_PTR_GET(callSiteData + callSiteIndex, J9ROMNameAndSignature *);

      // Null check!
      if (nameAndSig == NULL)
      {
         // printf("ERROR: nameAndSig is NULL\n");
         TR_ASSERT_FATAL(1, "ERROR: nameAndSig is NULL\n");
         // return;
      }

      J9UTF8 *callSiteName = J9ROMNAMEANDSIGNATURE_NAME(nameAndSig);
      J9UTF8 *callSiteSig = J9ROMNAMEANDSIGNATURE_SIGNATURE(nameAndSig);

      // printf("InvokeDynamic callsite: %.*s%.*s\n",J9UTF8_LENGTH(callSiteName), J9UTF8_DATA(callSiteName),J9UTF8_LENGTH(callSiteSig), J9UTF8_DATA(callSiteSig));

      std::string returnType = std::string(reinterpret_cast<const char *>(J9UTF8_DATA(callSiteSig)), J9UTF8_LENGTH(callSiteSig));

      size_t pos = returnType.find(')');
      returnType = returnType.substr(pos + 1);

      // 1. Parse the signature and POP the arguments first
      char *sigChars = (char *)J9UTF8_DATA(callSiteSig);
      ...
      if (returnType != "V")
      {
         PAGNode *retNode = new PAGNode(VARIABLE, -1, nullptr, method_block, bci, methodIndex);

         if (returnType == "J" || returnType == "D")
         {
            retNode->comp_type = "COMP_TYPE_2";
         }

         pag->PAG_nodes.insert(retNode);
         pag->methodIndex_to_allMethodNodes[methodIndex].push_back(retNode);

         retNode->pointee_class_names.insert(returnType);
         stack->pushRef(retNode);
      }

      // Now just find all lambda methods in the class
      // J9ROMMethod *currentMethod = J9ROMCLASS_ROMMETHODS(romClass);

      // for (U_32 i = 0; i < romClass->romMethodCount; i++)
      // {
      //    J9ROMNameAndSignature *methodNAS = J9ROMMETHOD_NAMEANDSIGNATURE(currentMethod);
      //    J9UTF8 *methodName = J9ROMNAMEANDSIGNATURE_NAME(methodNAS);
      //    J9UTF8 *methodSig = J9ROMNAMEANDSIGNATURE_SIGNATURE(methodNAS);

      //    // Check if this is a lambda method
      //    if (J9UTF8_LENGTH(methodName) >= 7 &&
      //        memcmp(J9UTF8_DATA(methodName), "lambda$", 7) == 0)
      //    {
      //       printf("Found lambda method: %.*s%.*s\n",
      //              J9UTF8_LENGTH(methodName), J9UTF8_DATA(methodName),
      //              J9UTF8_LENGTH(methodSig), J9UTF8_DATA(methodSig));

      //       // Get bytecodes
      //       U_8 *lambdaBytecodes = J9_BYTECODE_START_FROM_ROM_METHOD(currentMethod);
      //       U_32 bcLength = J9_BYTECODE_SIZE_FROM_ROM_METHOD(currentMethod);

      //       printf("  Lambda body: %u bytes\n", bcLength);

      //       // ANALYZE THE LAMBDA BYTECODES HERE
      //       // You now have the lambda implementation bytecodes!

      //       int32_t current_Index = 0;
      //       int statementCount = 0;
      //       operandStack *stack = new operandStack();

      //       while (current_Index < bcLength)
      //       {
      //          TR_ASSERT_FATAL(current_Index >= 0 && current_Index < bcLength, "Bytecode index out of bounds");

      //          uint8_t *pc = (uint8_t *)(lambdaBytecodes + current_Index);

      //          TR_J9ByteCode bytecode = TR_J9ByteCodeIterator::convertOpCodeToByteCodeEnum(*pc);
      //          int32_t instruction_Length = getInstructionLength(bytecode, pc);
      //          std::cout << "  [" << " INVOKEDYNAMIC " << "] PC: " << current_Index
      //                    << " - Opcode: 0x" << std::hex << (int)(*pc) << std::dec
      //                    << " - " << getBytecodeString(bytecode) << /*" ---> " << instruction_Length <<*/ std::endl;
      //          current_Index += instruction_Length;
      //          statementCount++;
      //       }
      //       // Example: Check if this lambda's return type matches the callsite
      //       // You can compare methodSig with callSiteSig to find the right lambda
      //    }
   ...
   // case J9BCinvokespecialsplit: // NEWLY ADDED
   // case J9BCinvokestaticsplit:  // NEWLY ADDED
   // {
   //    uint len = 0;
   //    J9Method *j9method;

   //    bool isStatic = (bytecode == J9BCinvokestaticsplit);
   ...
   //    sigChars = (char *)J9UTF8_DATA(signature_utf8);
   //    sigLength = J9UTF8_LENGTH(signature_utf8);
   //    nameChars = (char *)J9UTF8_DATA(name_utf8);
   //    nameLength = J9UTF8_LENGTH(name_utf8);
   ...
   //       J9RAMStaticMethodRef *ramStaticMethodRef = (J9RAMStaticMethodRef *)&cp[cpIndex];
   //       J9Method *resolved_Method = ramStaticMethodRef->method;

   //       J9Class *declaringClass = J9_CLASS_FROM_METHOD(resolved_Method);
   //       J9ROMClass *romClass = declaringClass->romClass;
   //       J9UTF8 *classNameWrapper = J9ROMCLASS_CLASSNAME(romClass);
   ...
   //       J9Class *declaringClass J9_CLASS_FROM_CP(ramConstantPool);
   ...
   //             { // this should never get executed for specialsplit and staticsplit as invokevirtual will be used

   //                std::cout << getBytecodeString(bytecode) << " here!!" << std::endl;
   ...
   case J9BCinvokehandle:        // NEWLY ADDED
   case J9BCinvokehandlegeneric: // NEWLY ADDED
   ...
   case J9BCinvokespecial:
   {
      ...
      bool isStatic = (bytecode == J9BCinvokestatic || bytecode == J9BCinvokestaticsplit);
      bool isInterfaceInvoke = (bytecode == J9BCinvokeinterface2);
      int callsiteBCI = bci;
      ...
      else
      {
         ...
         romMethodRef = (J9ROMMethodRef *)romCPItem;
      }
   ...
   case J9BCldc2lw:
   {
      stack->pushRef(comp_type_2); // push long constant
      break;
   }

   case J9BCldc2dw:
   {
      stack->pushRef(comp_type_2); // push double constant
      break;
   }
   ...
   case J9BCwide:
   {
      // The modified opcode is at pc[1]
      TR_J9ByteCode modifiedOpcode = TR_J9ByteCodeIterator::convertOpCodeToByteCodeEnum(pc[1]);

      // In both formats, the 16-bit index is formed by pc[2] and pc[3]
      uint16_t wideIndex = (pc[2] << 8) | pc[3];

      switch (modifiedOpcode)
      {
      case J9BCaload:
      {
         PAGNode *ref = variableMap[wideIndex];
         stack->pushRef(ref);
         break;
      }
      ...
      case J9BCastore:
      {
         ...
         if (!variableMap[wideIndex])
         {
            ...
            pag->PAG_nodes.insert(variableMap[wideIndex]);
            pag->methodIndex_to_allMethodNodes[methodIndex].push_back(variableMap[wideIndex]);
         }

         for (PAGNode *obj_ref : stack_top)
         {
            EdgeType e = (obj_ref->type == OBJECT) ? NEW : ASSIGN;
            pag->addEdge(obj_ref, variableMap[wideIndex], e, bci);
            variableMap[wideIndex]->pointee_class_names.insert(obj_ref->pointee_class_names.begin(), obj_ref->pointee_class_names.end());
         }
         break;
      }
      ...
      case J9BCiinc:
         // iinc_w has an additional 16-bit constant at pc[4], pc[5]
         // No stack effect or PAG impact for primitive increments
         break;

      case J9BCgenericReturn:
         // ret is used for subroutines (jsr), usually ignored in modern PTA
         break;

      default:
         TR_ASSERT_FATAL(false, "Invalid opcode modified by wide: %s", getBytecodeString(modifiedOpcode));
         break;
      }
      break;
   }
      ...
      if (method_signature == signature_name && method_name == target_method_name)
      {
         ...
         if (isAbstractMethod && className.rfind(StaticClassName) == 0)
         {
            if (calleeReturnsReference)
            {
               ...
               {
                  int m_ind = getOrInsertMethodIndexByName(full_method_name, pag);
                  PAGNode *retn = pag->getReturnNode(m_ind);
                  if (!retn)
                  {
                  ...
                  pag->addEdge(retn, returnNode, ASSIGN, bci);
               }
            }
            else if (calleeReturnsPrimitive)
            {
...
int32_t getInstructionLength(TR_J9ByteCode bytecode, uint8_t *pc)
{
   ...
   case J9BCasyncCheck:
   case J9BCbreakpoint:
   case J9BCunknown: // Typically treated as a 1-byte placeholder/error
      return 1;
   ...
   case J9BCinstanceof:
   case J9BCiinc:
   // Added missing split/handle opcodes (usually 3 bytes: opcode + 2 index bytes)
   ...
   case J9BCmultianewarray:
   // Added wide variants (usually 4 bytes: opcode + 2 index bytes + 1 extra)
   case J9BCiloadw:
   case J9BClloadw:
   case J9BCfloadw:
   case J9BCdloadw:
   ...
   // iincw is unique; it's often opcode + 2 index bytes + 2 constant bytes
   case J9BCiincw:
      return 6;
...
bool isSwitch(TR_J9ByteCode bc)
{
   return (bc == J9BCtableswitch || bc == J9BClookupswitch);
}

bool isReturnOrThrow(TR_J9ByteCode bc)
{
   switch (bc)
   {
...
int32_t branchTarget(int32_t pcIndex, TR_J9ByteCode bc, uint8_t *pc)
{
   if (bc == J9BCgotow)
   {
...
std::vector<int32_t> switchTargets(int32_t pcIndex, TR_J9ByteCode bc, uint8_t *pc)
{
...
TR::CFG *buildCFG(TR_OpaqueMethodBlock *method_block, TR::Compilation *comp, std::map<int32_t, TR::Block *> &blocks, TR::Block *&entryBlock)
{
   ...
   {
      // std::cout << "pcIndex= " << pcIndex << std::endl;
      ...
      pcIndex += len;
      // std::cout << "len is " << len << " After adding pcIndex= " << pcIndex << std::endl;
      if (len <= 0)
      {
         std::cout << "bc = " << bc << pcIndex << " - Opcode: 0x" << std::hex << (int)(*pc) << std::dec << " - " << getBytecodeString(bc) << " ---> " << len << std::endl;
      }
   }
   ...
   {
      // TR::Block *b = TR::Block::createEmptyBlock(0, comp, -1, (TR::Block *)__null);
      TR::Block *b =
          new (comp->trHeapMemory()) TR::Block(
              TR::TreeTop::create(comp, TR::Node::create(0, TR::BBStart)),
              TR::TreeTop::create(comp, TR::Node::create(0, TR::BBEnd)),
              comp->trMemory());
...
int getSlotForArgument(const char *descriptor, int argIndex)
{
   ...
   while (*p && *p != ')')
   {
      ...
      {
         // Array type: skip all '[' and next type (may be object or primitive)
         while (*p == '[')
            ++p;
         if (*p == 'L')
         {
